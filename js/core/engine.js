...
window.ENGINE.Core = (function () {
    ...
    function frame(mainCtx, overlayCtx, canvas, loop = true) {
        ...
        // --- Grid & Ruler (Overlay Layer) ---
        if (overlayCtx && config.showGrid) {
            ...
            overlayCtx.strokeStyle = "rgba(100, 100, 100, 0.4)"; overlayCtx.lineWidth = 1; overlayCtx.beginPath();
            for (let i = 0; i < gridOut.length / 8; i++) {
                let i8 = i * 8; if (gridOut[i8 + 3] > 0 && gridOut[i8 + 7] > 0) { overlayCtx.moveTo(gridOut[i8], gridOut[i8 + 1]); overlayCtx.lineTo(gridOut[i8 + 4], gridOut[i8 + 5]); }
            }
            overlayCtx.stroke();
            ...
            Rasterizer.drawRuler(overlayCtx, rulerOut);

            // --- FRONT DIRECTION TOOLTIP ---
        ...
        if (useGL) {
            // Unified Perspective (45 degrees default)
            const fovy = config.fov * Math.PI / 180;
            mat4.perspective(mProj, fovy, canvas.width / canvas.height, 0.1, 1000.0);
            ...
            MathOps.projectBuffer(buffers.screen, vCount, canvas.width, canvas.height, config.fov * 13.33);
            ...
            if (config.viewMode !== 'POINTS') {
                ...
                Rasterizer.drawPoints(mainCtx, buffers.screen, vCount, config);
            }
        }
        ...
        const fovScale = config.fov * 13.33; // Synchronized with Grid/Ruler (approx 600 at 45 deg)
        const origin = [object.pos.x, object.pos.y, object.pos.z];

        if (overlayCtx) {
            if (GR && state.ui.hoveredObjectId && !state.ui.selectedObjectId && buffers && buffers.screen) {
                // Note: Hover outline currently needs buffers.screen which is CPU-only
                if (!useGL) GR.drawHoverOutline(overlayCtx, buffers.screen, indices, fCount, '#66ccff');
            }
